...
is_completed = False
# is_completed --> game round controller variable
...
class Person:
    """
    Buleprint of a player in the game.

    Attributes:
        name (str): The player's name.
        stocks (List[str]): A list of the player's stock holdings, formatted as "[stockName(lower)]D[stock_volume]D[stock_price]".
        amount (float): The player's current cash amount (liquidity available to the player).
        net_worth (float): The player's net worth (cash + stock value(now)).
        order (List[Dict[str, str | float | int]]): A list of the player's orders. <completed not completed orders>
        id (str): The player's unique ID.<yet not used much>
    """

    name: str
    stocks: List[str]
    # [stockName(lower)]D[stock_volume]D[stock_price]
    amount: float
    net_worth: float
    ...
    @override
    # overriding the initially available print(person) so that print works
    # in a  predefined way
...
class Stock:
    ...
    Attributes:
        ...
        volume (int): The total number of shares available [market + in player holdings]
        capital (float): The total capital of the stock [capital intital + capital transacted on the stock (buy-sell)]
        price (float): The current price per share [volume / capital]
